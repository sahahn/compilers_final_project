...
def eval_rlam(program: RlamProgram):
    def eval_e(e: RlamExp, env: Dict[str, Any]):
        if isinstance(e, Int):
            return e.val
        if isinstance(e, Bool):
            ...
            # If dynam, update any overlapping values
            if f_val.name == 'dynam':
                for key in extended_env:
                    if key in env:
                        extended_env[key] = env[key]

            return eval_e(f_val.body, extended_env)

        elif isinstance(e, Lambda):
            arg_names, arg_types = unzip2(e.args)
            return FunVal('lambda', list(arg_names), env, e.body)

        elif isinstance(e, Dynam):
            arg_names, arg_types = unzip2(e.args)
            return FunVal('dynam', list(arg_names), env, e.body)
